...
Move the bow while left clicking and holding.`}}class tA extends eA{constructor(A){let e=[],a=!0;for(let A=0;A<200;A+=15)e.push({x:C-20-A,y:U+150,speed:a?3:2}),a=!a;super(A,25,e),this.title="More balloons",this.description="You might want to wait until they form a single line."}}class oA{constructor(A){this.arrows_limit=12,this.bubbles=[],this.butterflies=[],this.onlevelend=A;let e=!0;for(let A=0;A<240;A+=24)this.bubbles.push({x:C-30-A,y:e?0:U-AA,speed:-2,frame:0}),e=!e;this.bg_image=G,this.title="Bubbles",this.description="Pop bubbles to rescue butterflies!"}draw_on(A){for(let e of this.bubbles)A.drawImage(_,$*e.frame,0,$,AA,e.x,e.y,$,AA);for(let e of this.butterflies)A.drawImage(j,z*e.frame,0,z,P,e.x,e.y,z,P)}update(A){this.bubbles=this.bubbles.filter(e=>{for(let a of A){let A=a.y+2-e.y,t=a.x+Z-e.x;if(!e.pierced&&A>=0&&A<=7&&t>=4&&t<=7||A>=8&&A<=11&&t>=2&&t<=5||A>=12&&A<=23&&t>=0&&t<=3||A>=24&&A<=27&&t>=2&&t<=5||A>=28&&A<=31&&t>=4&&t<=7||A>=32&&A<=42&&t>=6&&t<=9){o(DA),l(10),e.pierced=!0,this.butterflies.push({x:e.x-1,y:e.y+12,frame:0});break}}return e.pierced?++e.frame:(e.y-=e.speed,(e.y<=-4||e.y>U-AA+4)&&(e.speed=-e.speed)),!(e.pierced&&e.frame>=9)}),this.butterflies=this.butterflies.filter(A=>{return A.x-=2,A.y-=2,A.frame=(A.frame+1)%8,A.y>-P}),0==this.bubbles.length&&0==this.butterflies.length&&g(!0)}can_safely_fail(){return this.bubbles.length>0&&0==this.butterflies.length}}let iA=Object.freeze({INACTIVE:1,HIDDEN:2,HIDING:3,SHOWING:4,OTHER:5});class rA{constructor(A,e){this.bg_image=G,this.arrows_limit=e,this.onlevelend=A,this.violets=[];let a=[];for(let A=0;A<4;++A)this.violets.push({x:C,y:45+93*A,health:10,state:iA.INACTIVE}),a.push([A,Math.random()]);this.inactive=a.sort((A,e)=>A[1]<e[1]).map(A=>A[0]),this.release_violet(),this.game_over=!1}release_violet(){let A=this.inactive.pop();void 0!=A&&(this.violets[A].state=iA.HIDDEN)}draw_on(A){for(let e of this.violets){A.fillStyle="#87238f";let a=e.y-8;for(let t=e.x+10,o=0;o<e.health;++o,t+=6)A.fillRect(t,a,5,2);A.drawImage(O,N,x,v,L,e.x,e.y,v,L)}}check_if_level_end(){0==this.violets.length?g(!0):this.game_over&&g(!1)}update(A){this.violets=this.violets.filter(e=>{if(e.state==iA.SHOWING)for(let a of A){let A=a.y+2-e.y,t=a.x+Z-e.x;if(A>=-2&&A<=1&&t>=34&&t<=37||A>=2&&A<=3&&t>=28&&t<=31||A>=4&&A<=5&&t>=26&&t<=29||A>=6&&A<=11&&t>=22&&t<=25||A>=12&&A<=13&&t>=8&&t<=11||A>=14&&A<=15&&t>=2&&t<=5||A>=16&&A<=29&&t>=0&&t<=3||A>=30&&A<=31&&t>=2&&t<=5||A>=32&&A<=33&&t>=4&&t<=7||A>=34&&A<=37&&t>=8&&t<=11||A>=38&&A<=51&&t>=6&&t<=9||A>=52&&A<=55&&t>=8&&t<=11||A>=56&&A<=57&&t>=10&&t<=13||A>=58&&A<=59&&t>=14&&t<=17||A>=60&&A<=61&&t>=44&&t<=47||A>=62&&A<=65&&t>=46&&t<=49){o(DA),l(10),--e.health,e.state=iA.HIDING,this.release_violet();break}}switch(e.state){case iA.HIDDEN:e.state=iA.OTHER,setTimeout(A=>{e.state=iA.SHOWING},1e3);break;case iA.SHOWING:e.x-=1;break;case iA.HIDING:e.x<C?e.x+=3:e.state=iA.HIDDEN}return e.x<=-v/2&&(this.game_over=!0),!(e.health<=0&&e.state==iA.HIDDEN)}),this.check_if_level_end()}can_safely_fail(){return this.violets.length>0&&0==this.violets.filter(A=>A.health<=0).length}}class cA extends rA{constructor(A){super(A,45),this.title="Bad weather",this.description=`Some angry clouds are on their way.
